...
C - Tiago Ribeiro
    ...
    template = [pymc.DiscreteUniform('template_%02i'%(i+1),
                                     lower=gridmin[i],
                                     upper=gridmax[i],
                                     value=val[i]) for i in range(np.sum(spMod.grid_ndim))]
...
2 - SDSSFits, 3 - Coelho 2014 model spectra''',type='int',default=1)
    ...
    #ovars = ['scale','velocity']
    #for i in range(np.sum(M.spMod.grid_ndim)):
    #    ovars.append('template_%02i'%(i+1))
    ...
    for i in range(M.ncomp):
        ...
        for j in range(M.spMod.grid_ndim[i]):
            oarray['temp%i_%i'%(i,j)] = np.array(	[ii for ii in M.trace('template_%02i'%(i*2+j+1))[:]] )
    ...
    if opt.savefig:
        ...
        py.savefig(os.path.join(pltdir,os.path.basename(plotname)+'.png'))
